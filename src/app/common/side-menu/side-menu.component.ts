...
export class SideMenuComponent implements OnInit {
  ...
  menuItemList = [
    ...
    },
    {
      ...
      role: ['Administrator', 'Cashier', 'Driver', 'Scale']
    },
    // {
    //   title: 'Sales Order',
    //   url: '/sales-order',
    //   icon: '/assets/images/custom/icons/SalesOrder.png',
    //   role: ['Administrator', 'Cashier']
    // },
    // {
    //   title: 'Purchase Order',
    //   url: '/purchase-order',
    //   icon: '/assets/images/custom/icons/PurchaseOrder.png',
    //   role: ['Administrator', 'Cashier']
    // },
    {
      title: 'Materials',
    ...
    //   title: 'Truck',
    //   url: '/truck',
    //   icon: '/assets/images/custom/icons/TruckC.png',  
    //   role: ['Administrator', 'Cashier']
    // },
    {
      title: 'Regrade',
  ...
  ngOnInit() {
    ...
    const isProPlusVersion = localStorage.getItem('isProPlusVersion') === 'true';
    const isDispatchOnly = localStorage.getItem('isDispatchOnly') === 'true';
    if (!isProPlusVersion && isDispatchOnly && (this.currentRole === 'Administrator' || this.currentRole === 'Cashier')) {

      this.menuList = this.menuItemList.filter((el) =>
        ['Dispatch', 'Sellers/Buyers', 'Reports', 'Admin'].includes(el.title)
      );
    }
    else if (!isProPlusVersion) {
      this.menuList = this.menuItemList.filter(function (el) {
        return (el.title.toString() != "Invoice"
          && el.title.toString() != "Dashboard"
          && el.title.toString() != "Internal Transfer"
          && el.title.toString() != "Container"
