...
export class InvoiceLayoutComponent implements OnInit {
  ...
  navigateToInitialView() {
    ...
    this.isInvoiceMode = currentUrl.includes('invoice') && !currentUrl.includes('grid');
  }
  ...
  onToggleChange() {
    if (this.isInvoiceMode) {
      ...
      this.router.navigate([`/${this.orgName}/invoice/grid`]);
    }
  }
  ...
  // }
  addRemoveStatus(event: any) {
    console.log('Change Multiselect :: ', event);
    ...
    if (event.itemValue.name === 'ALL') {
      if (event.originalEvent) {
        ...
        this.selectedTickets = this.defaultSelectedTicketsTypes;
        selectedStatus = this.defaultSelectedTicketsTypes.map((item: any) => item.name).join(',');
      }
    }
  
    this.router.navigate([], { queryParams: { status: selectedStatus },});
  }
