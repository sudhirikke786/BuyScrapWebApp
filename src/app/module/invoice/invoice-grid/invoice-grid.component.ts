...
export class InvoiceGridComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      this.pagination.Status = params['status'] || this.selectedTickets.map((t: any) => t.name).join(',');
      this.getAllTicketsDetails(this.pagination);
    });

    const storedPagination = localStorage.getItem('invoicePaginationGridData');
    if (storedPagination) {
  ...
  getAllTicketsDetails(pagination: Pagination) {
    ...
    this.commonService.GetAllInvoiceDetails(pagination)
      .subscribe(data => {
        ...
        localStorage.setItem('invoicePaginationGridData', JSON.stringify(this.pagination));
