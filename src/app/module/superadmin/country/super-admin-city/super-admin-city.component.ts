...
@Component({
  selector: 'app-super-admin-city',
  templateUrl: './super-admin-city.component.html',
  styleUrls: ['./super-admin-city.component.css'],
  providers: [MessageService]
})
export class SuperAdminCityComponent implements OnInit{
  country: any = {};
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      this.stateID = +params['stateID'] || 0;
      if (this.stateID > 0) {
        this.getAllCity();
      }
    });
    console.log("Checking CountryID",this.stateID)
  }

  getAllCity(searchText:string='',countryID:number = 0) {
    const paramObj = {
      CountryID: countryID,
      StateID: this.stateID,
      SearchText: searchText
    };
    this.commonService.getAllCity(paramObj).subscribe(
      (data) => {
  ...
  this.commonService.InsertUpdateCity(requestObj).subscribe({
    next: (response) => {
      if (response.body) {
  ...
  back(){
    this.router.navigateByUrl(`/superadmin/country/superadmin-state`);
  }
}
