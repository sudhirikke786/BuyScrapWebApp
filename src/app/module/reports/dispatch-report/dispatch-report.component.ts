...
export class DispatchReportComponent implements OnInit {
  ...
  getDispatchReportData(reportType? : string | null) {
    ...
    this.commonService.getDispatchReportData(param)
      .subscribe(data => {
        ...
        if(this.checkTabView && !reportType) {
          this.helperService.downloadBase64Pdf(this.fileDataObj,"Dispatch Report "+this.toDate);
        } else if (reportType && reportType != 'PDF') {
          this.helperService.downloadBase64Report(this.fileDataObj,"Dispatch Report "+this.toDate, reportType);
        }
  ...
  getAction(actionCode: any) {
    ...
    switch (actionCode?.iconcode) {
      ...
      case 'mdi-file-pdf-box':
        this.getDispatchReportData('PDF');
      break;
      case 'mdi-file-excel-box':
        this.getDispatchReportData('Excel');
        break;
      case 'mdi-file-word-box':
        this.getDispatchReportData('Word');
        break;
        case 'mdi-xml':
          this.getDispatchReportData('XML');
          break;
      default:
        break;
    }
