...
import { PurchaseOrder } from 'src/app/core/model/purchase-order.model';
import {PurchaseOrderItem} from 'src/app/core/model/purchase-order-item.model';
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe((param) => {
      ...
      this.route.queryParams.subscribe(params => {
        ...
        this.purchaseOrderId = Number(param["purchaseOrderId"]);
        const type = params['type']; 
        this.isViewShip = (type === 'viewship');

      if (this.isViewShip) {
        console.log('ViewShip mode detected');
        this.getTicketMaterials(this.purchaseOrderId);
      } else {
        this.getPurchaseOrderMaterials(this.purchaseOrderId);
      }
    ...
    this.salesOrderData = this.salesOrderData || new PurchaseOrder();
    ...
    this.salesOrderData = new PurchaseOrder();
...
// }
  ...
  savePurchaseOrderDetails() {
    ...
    const payload = {
      ...
      lstPurchaseOrderMaterials: this.ticketObj.map((item: any) => ({
        rowID: item.rowID || 0,
    ...
    this.commonService.InsertUpdatePurchaseOrder(payload).subscribe({
      next: (res: any) => {
      ...
      setTimeout(() => {
        this.router.navigateByUrl(`/${this.orgName}/purchase-order`);
      }, 1500);
  ...
  updateExistingItemDataResponse() {
    ...
    if (this.itemLeveloperationPerform === 'Add') {
      // const arr = [];
      const rowData = new PurchaseOrderItem();
      rowData.rowID = 0;
  ...
  getPurchaseOrderById() {
    const paramObject = {
      RowID: this.purchaseOrderId
    };
    this.commonService.GetPurchaseOrderById(paramObject).subscribe({
      next: (res: any) => {
  ...
  getPurchaseOrderMaterials(purchaseOrderId: number) {
    this.isLoading = true;
    ...
    this.commonService.GetPurchaseOrderMaterialDetailsByPurchaseOrderID(paramObj).subscribe({
      next: (res: any) => {
        console.log('API Response:', res); 
  ...
  getTicketMaterials(purchaseOrderId: number) {
    this.isLoading = true;

    const paramObj = { 
      PurchaseOrderID: purchaseOrderId
    }; 
    this.commonService.GetTicketsMaterialsByPurchaseOrderID(paramObj).subscribe({
      next: (res: any) => {
        console.log('ShipOut Materials Response:', res);
        this.TicketMaterials = res?.body?.data || [];
        this.isLoading = false;
      },
      error: (err) => {
  ...
  console.log('Calling GetPurchaseOrderDetailsByID API with:', paramObj);

  this.commonService.getPurchaseReportByID(paramObj).subscribe({
    next: (response) => {
      console.log('GetPurchaseOrderDetailsByID Response:', response);
      if (response?.body?.data) {
        console.log('Data received successfully:', response.body.data);
      } else {
